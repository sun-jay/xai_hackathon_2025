...
export async function POST(request: NextRequest) {
  try {
    ...
    const replica_id = body.replica_id || process.env.TAVUS_REPLICA_ID
    const persona_id = body.persona_id || process.env.TAVUS_PERSONA_ID

    console.log('🔍 API Received config:', {
      body_replica_id: body.replica_id,
      body_persona_id: body.persona_id,
      env_replica_id: process.env.TAVUS_REPLICA_ID,
      env_persona_id: process.env.TAVUS_PERSONA_ID,
      final_replica_id: replica_id,
