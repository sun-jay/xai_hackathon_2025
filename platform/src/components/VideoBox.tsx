...
export function VideoBox({ conversationUrl, isLoading = false }: VideoBoxProps) {
        ...
        case 'check_diagram':
          ...
          try {
            ...
            if (!response.ok) {
              ...
              result = `Error checking diagram (Status ${response.status}): ${errorText}`
            } else {
              const data = await response.json()
              result = data.feedback || JSON.stringify(data)
            }
          } catch (err) {
